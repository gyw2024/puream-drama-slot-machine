...
test("packaged and discoverable adaptation skill have identical instructions",()=>{const deployed="D:/CodexData/.codex/skills/puream-script-adaptation/SKILL.md";if(fs.existsSync(deployed))assert.equal(fs.readFileSync(deployed,"utf8").replace(/\r/g,"").trim(),a.skill.trim());assert.ok(a.skill.includes("每5镜独立任务并行只适用于后续分镜视频提示词生成"));});
