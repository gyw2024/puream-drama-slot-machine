...
for(const mode of ['original','upload','adapt'])test(mode+': independent user product facts survive a conflicting reviewer proposal across author/review/repair',async()=>{
 ...
 const result=await screenplay.author({mode,source:mode==='original'?'':'旧稿里声称清热益气，尚待核对。',product,generate:async(m,o)=>{
  ...
  if(o.stage==='shot_screenplay_write')return structuredClone(d);
 ...
 assert.equal(result.status,'ready');assert.deepEqual(calls,['shot_screenplay_write','shot_screenplay_review','shot_screenplay_repair','shot_screenplay_review']);
});
