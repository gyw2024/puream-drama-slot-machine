...
function schema(project,shots){
 ...
 // The generic "this exact object" clause is stated once at the schema root
 // (PROP_IDENTITY_NOTE) instead of being copied into every per-prop field.
 const stateFields=prop=>Object.fromEntries(['openingEn','openingZh','endingEn','endingZh'].map(key=>[key,{...(key.endsWith('En')?narrative:text),description:'State of '+prop.id+' ('+prop.name+') only.'}]));
 ...
 result.description='Per-shot director execution contract. Every state, objectState and narrative field describes its exact declared ID only: one characterId is one physical person and one propId is one physical object, never another salient scene object or the original product.';
