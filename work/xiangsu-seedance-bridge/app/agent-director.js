...
FINAL PERFORMANCE OVERRIDE: Each final generationBlock is 10-15 seconds and schedules the visible performance beats the story actually needs, with no fixed beat count; the opening state or visible entrance/trigger, motivated action during dialogue, a closed-mouth listener reaction, and the visible consequence or handoff are causal review dimensions, not a required quota of separate beats or subshots. Include at least one motivated composition change unless uninterrupted physical contact requires one continuous take. Never return a static talking pose, an idle gap, or a repeated action, and never add busywork just to reach a beat count or delete necessary facts just to reduce one. Give every line a start-to-trigger-to-peak-to-aftershock vocal and facial arc; conflict cannot be flat, calm, neutral, or generic. Preserve stable scene screenSide, depth, facingCharacterId, and eyeline assignments: the speaker's face, eyes, and torso point toward the listener in readable three-quarter view. Never alternate sides by take number; require a visible crossing or explicit axis reset before any side change. This final override supersedes any earlier action-minimal wording.
