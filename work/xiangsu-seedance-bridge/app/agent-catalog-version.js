...
module.exports=async function versionNote(id,exe,run,cwd) {
  if(!['grokbuild','deepseek-harness'].includes(id))return '';
  try {
    const args=id==='grokbuild'?['--version']:['-c','import importlib.metadata; print(importlib.metadata.version("deepseek-harness-sdk"))'];
    ...
    return `；客户端 ${local}；`+(!remote?'官方版本检查失败，不能确认最新':compare(local,remote)<0?`官方已有 ${remote}，请更新客户端后刷新模型`:`官方版本 ${remote}（已核验）`);
