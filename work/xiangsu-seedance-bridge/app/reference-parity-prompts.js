...
const REFERENCE_PARITY_PROMPT_VERSION = "2026.08-continuity-block-v10";

const CONTINUITY_BLOCK_OVERRIDE = `【H3连续剧情块最终覆盖规则】一个Sxx是5–15秒剧情块，不是固定单人机位；最多2名说话人的自然问答保留在同一Sxx。每次speakerId变化都形成明确时间码HARD CUT，并同步切换cameraOwnerId/mouthOwnerId，当前说话人开口、听者闭口。导演Agent把轮次编译为原子机位段后，优先合并成同一个H3视频任务；只有提示过长、上游拒绝或成片缺少预期切镜时，才局部回退为原子任务并精确拼接。禁止BGM、字幕、人物介绍、错嘴、溶解变脸和参考板入片。`;
...
按目标时长等比计算并逐项给出镜号/秒点：一句话可复述主线；前8秒可见危机、前35秒现实代价；机制不被强改成证据谜题；每30–75秒有新任务或有动机换场；唯一主反转位于65%–80%；80–100个可剪辑subshots、60–90轮对白。逐镜检查：整镜可见人物≤2、单人镜占比≥50%、每个subshot只有一个主口型角色、场内第三人未被错误加入当前画面/音色、人物情绪有可见肌肉与身体动作、切镜有动作/视线/声音动机。商品必须有独立整体/细节/使用/结果镜，整体与细节镜无抢镜人脸。剧情难以复述、连续多人关系全景、同场景占比过半、未知说话人、产品被三人围挡或无特写，均为 hardFailure。`,
    ...
    referenceParitySeedanceVideo: `【十部参考成片同规格·Seedance本镜硬合同·全模式】严格按图/视频/音频编号使用参考：首尾帧、上一视频或逐秒合图只控制本镜时间锚，人物/服装/道具/商品图只控制对应资产，音频只控制同名角色音色；场景图是2×2四角度板，只选与本镜机位匹配的一格来锁门窗家具与光向，最终视频禁止出现四宫格、边框或参考板。完整执行0→duration的动作与对白顺序，优先级不可改：对白内容＞语气＞情绪＞场景＞运镜＞其他。每句对白逐字完整说完，先锁说话人和音色，再执行语气、情绪、重音、气口、面部和身体动作；说话人看听者且仅其开口，听者闭嘴并给出可见反应，不对镜头。镜头必须从首态推进到肉眼不同的尾态；三段/多格动作、表情和身体必须互异，禁止等分空镜与整板复制。禁止把时间码、场景标题、动作或商品制作说明当对白，禁止复读、串角、吞句或用泛化情绪替代原文。禁止复刻参考板、重复前镜构图、人物换脸换装、空间反转、商品早泄。环境底噪与同步SFX覆盖全段，头尾不得掉声；禁止背景音乐/BGM。首尾帧模式走首→尾因果链；延续模式无缝承接上一视频末态；合图模式按格序演绎且禁止格线序号入成片。`,
...
- The block may contain one to five timed camera segments. Each segment locks exactly one cameraOwnerId and at most one mouthOwnerId/speaker. Every speaker change creates an explicit HARD CUT inside this same provider task and switches both owners together.
...
- Dialogue/eyeline/matched-action/object/entrance/sound motivates each internal HARD CUT. Use a separate provider task only as a validated fallback for prompt/provider limits or missing-cut QC. Keep micro-motion through the final frame.
- Continuous bed + synced SFX only; nonDiegeticMusicEn always N/A; never BGM/underscore. Product packshot/detail: product 45–75%, no extra face/hand. English JSON only.`,
    ...
    referenceParityHailuoVideo: `REFERENCE-DIRECTOR PARITY — FINAL CONTINUITY BLOCK (DRAMA FIRST, ALL MODES): render one 5–15s irreversible live-action block with rising performance intensity (start→trigger→peak→aftershock). Priority is immutable: exact Chinese dialogue content > delivery/tone > emotion > scene continuity > camera movement > other decoration. Follow the supplied timed camera segments in order. Each segment has one cameraOwnerId, one mouthOwnerId and one exact Chinese voice; a speaker change is an explicit HARD CUT inside this same task and switches camera and mouth ownership together. At every instant the listener remains silent with closed lips and reacts. Execute authored face/body/voice change, preserve every exact line once without truncation or paraphrase, continuous bed/SFX only (no BGM/underscore), and micro-motion through the end. Use only the matching panel of the scene reference to lock doors, windows, furniture and light; never render boards, gutters, labels or UI. Keyframe / continuation / storyboard-sheet only change temporal anchors. Never flatten acting, freeze, show subtitles, add a third face, dissolve, morph or invent an establishing reset.`
  };
  return {
    ...templates,
    ...
    referenceParityShotPlan: `【参考成片分镜合同】每个Sxx是5–15秒连续剧情块，只做一条“因X→说/做Y→导致Z”的状态改变。scenePresence不等于入画；visibleCharacterIds最多2人，第三人另开反应/入场剧情块。保留最多2名说话人的自然问答与原始轮次；每句对白必须是完整口语原文，绝不把时间码、场景标题、动作、商品说明或制作说明当台词。对白质量优先级固定为：对白内容＞语气＞情绪＞场景＞运镜＞其他。每句同时给出说话人、听者、语气、情绪峰值、重音/气口、面部身体动作和听者反应；禁止全镜复用同一套情绪模板。顶层cameraOwnerId/mouthOwnerId只表示开场机位，不限制整段。每次speakerId变化都必须形成带时间码、cutReason、cameraOwnerId和mouthOwnerId的明确正反打，当前说话人开口、听者闭口反应。导演Agent先编译原子机位段，再优先合并为一个H3连续视频任务；仅在提示过长、上游拒绝或切镜质检失败时局部回退。商品按整体/细节/品类动作/客观结果/受益者反应拆职责。`,
    referenceParityUnits: `【参考成片制作单元合同·全模式】每个Sxx保留恰好3个不等长宏观subshots；对白轮次可在subshot内部继续细分为带时间码的原子机位段。只用锁定的0–2名可见人物；每个时间段只能有一个mouthOwnerId开口，speakerId变化时同步HARD CUT到对应cameraOwnerId，允许同一Sxx内完成自然正反打。dialogueTurns按attack/deflect/counter/reveal/decision推进，句句新增信息或改变权力/行动；text只放原始完整台词，delivery、body和listenerBeat写可执行表演。对白内容必须先于语气、情绪、场景、运镜和其他描述进入提示词；禁止时间码、场景标题、动作说明、商品说明、制作备注进入text。每句必须有不同的情绪/气口/身体落点，禁止全剧套用同一情绪模板。导演Agent将全部机位段编译成一个≤15秒H3连续生成块；逐秒合图随时间继承相应机位和嘴型所有权，不得把参考板、格线或标签带入成片。bed与同步SFX覆盖全段，禁止背景音乐、字幕、人物介绍。`
  };
}
...
function referenceParityFor(prompts, stage) {
  ...
  return CONTINUITY_OVERRIDE_KEYS.has(key)
    ? [value, CONTINUITY_BLOCK_OVERRIDE].filter(Boolean).join("\n\n")
    : value;
}
