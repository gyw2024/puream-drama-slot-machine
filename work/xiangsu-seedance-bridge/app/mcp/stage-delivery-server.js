...
function deliveryInputSchema(request){
 ...
 const advertised=request.progressiveDelivery
  ...
  :{type:'object',additionalProperties:false,properties:{data:request.json&&request.responseSchema?request.responseSchema:{},text:{type:'string'},useStaged:{type:'boolean'},file:{type:'string',enum:['result.json','result.txt']}}};
...
function partInputSchema(request){
 ...
 const advertised={type:'object',additionalProperties:false,properties:{field:{type:'string',...(fields.length?{enum:fields}:{})},index:{type:'integer',minimum:0},data:entries.length?{anyOf:entries.map(([,schema])=>require('./stage-parts').fragmentSchema(schema))}:{},read:{type:'boolean'}},required:['field','index'],description:'data is the actual typed value of the selected field. Object fragments retain real property names and nested structures; array fragments are arrays. Do not quote an object as text. Fragments remain in this one task. Object fragment indexes are storage addresses; arrays retain consecutive ordering.'};
 return {'~standard':{version:1,vendor:'puream',validate:value=>envelope['~standard'].validate(value),jsonSchema:{input:()=>advertised,output:()=>advertised}}};
}
