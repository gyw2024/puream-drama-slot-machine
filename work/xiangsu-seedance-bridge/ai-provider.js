...
async function providerFetchOpenAiStream(url, options = {}, timeoutMs = 180_000, streamOptions = {}) {
  ...
  let idleTimer = null;
  let maxTimer = null;
  // A text-stage timeout is its hard upper bound.  Previously the generic
  // reader silently expanded this to four times the requested limit, which
  // left every OpenAI-compatible provider (not just Kimi) stuck in
  // "generating" for up to sixteen minutes.
  const requestedMaxTimeoutMs = Number(streamOptions.maxTimeoutMs);
  const maxTimeoutMs = effectiveTimeoutMs
    ? Math.max(effectiveTimeoutMs, Number.isFinite(requestedMaxTimeoutMs) && requestedMaxTimeoutMs > 0
...
function normalizedMaxTokens(config, fallback = 16384, requestedMaxTokens = 0) {
  ...
  const providerMaximum = /ark\.cn-[a-z0-9-]+\.volces\.com\/api\/coding\/v3/.test(baseUrl) ? 128000 : 131072;
...
function structuredJsonContinuationText(error) {
  ...
  if (!match) return "";
  return raw.slice(match[0].length - 1, 200_000);
}

function structuredJsonContinuationMessages(messages, partialJson, attempt) {
  return [
...
async function generateText(config, messages, options = {}) {
  if (options?.json !== true || options?.autoContinueJson === false) {
    return generateTextOnce(config, messages, options);
  }
  let partialJson = "";
  ...
  while (true) {
    ...
    if (!continuationAttempt) {
      try {
        const resetMessages = resetAttempt
          ? structuredJsonResetMessages(messages, resetAttempt, "无正式 JSON")
          : messages;
        return await generateTextOnce(config, resetMessages, {
          ...options,
          sessionId: resetAttempt
            ? `${String(options.sessionId || "text").slice(0, 140)}-json-reset-${resetAttempt}`
            : options.sessionId
        });
      } catch (error) {
        ...
        if (!shouldResetStructuredJson(error)) throw error;
        resetAttempt += 1;
        continue;
      }
    }
    const continuationMessages = structuredJsonContinuationMessages(messages, partialJson, continuationAttempt);
    let suffix = "";
    try {
      suffix = await generateTextOnce(config, continuationMessages, {
        ...options,
        json: false,
      ...
      const interruptedSuffix = structuredJsonContinuationText(error);
