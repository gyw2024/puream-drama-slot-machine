...
- 切流后进行了四轮间隔稳定性审计：`inv-h7dti90gmd`、`inv-m7dtjag1th`、`inv-j7dtkfgqrb`、`inv-f7dtuv081r`，每轮均确认唯一正式服务为 r91、Nginx 9/9、页面版本、完整下载哈希、四项无写探针、`NRestarts=0` 和严重日志 0；最后一轮时间为 2026-08-15 11:40:44（Asia/Shanghai）。
...
- 因此最终 r91 同时保留合作方任务功能和本次短剧 0.14.2 发布内容；连续四轮稳定性审计后才关闭发布。
